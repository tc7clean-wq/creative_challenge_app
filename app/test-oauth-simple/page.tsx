    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 flex items-center justify-center p-8">
      <div className="max-w-md w-full bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20">
        ...
        <button
          ...
          disabled={loading}
          className="w-full bg-white text-gray-900 py-3 px-6 rounded-lg font-semibold hover:bg-gray-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
        >
          {loading ? (
            <>
        ...
        <div className="mt-6 text-center text-sm text-gray-300">
          ...
          <p className="mt-2">Redirect will go to: {typeof window !== 'undefined' ? `${window.location.origin}/auth/callback` : 'Loading...'}</p>
        </div>
      </div>
    </div>
  )
